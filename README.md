...
A：基于变分上界（$L_{\text{VLB}}$）的化简，我们知道模型最核心的训练项 $L_{t-1}$，本质上是在逼近一个已知原图 $\mathbf{x}_0$ 时的单步后验高斯分布的均值。但在工程实现中，将 “预测均值” 等价转换为了 “预测噪声”。因此，U-Net 在训练阶段的唯一目标就是：在给定带噪图像 $\mathbf{x}_t$ 和当前时间步 $t$ 的情况下，盲猜出人类在这一步给它加的高斯白噪声 $\boldsymbol{\epsilon}$ 是什么。

Q：3. 逆向生成（Sampling）：1000 步的挤牙膏式去噪在真正的生成阶段（测试时），我们手里没有原图，只有一张从正态分布里随机抽取的纯噪声图 $\mathbf{x}_T$。我们通过一个倒序的循环（从 $T$ 到 $1$），一步步还原图像：预测噪声： 将当前的图 $\mathbf{x}_t$ 和时间步 $t$ 送入 U-Net，网络输出它猜测的纯噪声分量。计算均值基底： 利用我们手撕出来的“绿色均值公式”，把网络猜出来的噪声代入，计算出下一步图像的基准均值 $\tilde{\boldsymbol{\mu}}_t$。注入随机扰动（关键）： 为了保证生成图像的多样性和细节（郎之万动力学），我们不能直接把均值当成下一步的图像，而是必须加上一个由固定方差决定的微小随机噪声 $\sigma_t \mathbf{z}$（最后一步除外）。循环往复： 得到 $\mathbf{x}_{t-1}$ 后，再次送入网络。如此循环 1000 次，纯正的随机噪声最终褪尽铅华，化作一张高清、逼真、且独一无二的生成图像 $\mathbf{x}_0$。